...
- When adding translations to the desktop or web app, first use <FormattedMessage> or useIntl in the relevant component. Then run the `i18n:extract` script in the desktop app to extract messages. Finally, run the `i18n:sync` script to sync new messages to all locale files. Lastly, translate the new messages in each locale file.
