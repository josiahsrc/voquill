...
fn main() {
    ...
    let whisper_root_is_empty = whisper_root
        .read_dir()
        .map(|mut iter| iter.next().is_none())
        .unwrap_or(true);

    if whisper_root_is_empty {
        let mut options = CopyOptions::new();
        options.overwrite = true;
        options.copy_inside = false;

        fs_extra::dir::copy("./whisper.cpp", &out, &options).unwrap_or_else(|e| {
            panic!(
        ...
        {
            panic!(
                "Whisper sources were not copied into {} (directory is empty)",
                whisper_root.display()
            );
        }
    }
