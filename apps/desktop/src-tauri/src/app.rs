            ...
            #[cfg(desktop)]
            {
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    if should_init_whisper {
                        log::info!("Transcription mode is local or unset, initializing Whisper in background...");
                        if let Err(err) =
                            initialize_transcriber_background(&app_handle_for_bg).await
                        {
                            log::error!("Background Whisper initialization failed: {err}");
                        }
                    } else {
                        log::info!(
                            "Transcription mode is '{}', skipping Whisper initialization",
...
async fn initialize_transcriber_background(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    log::info!("Background Whisper initialization completed successfully");
