...
export const RootSideEffects = () => {
  const [currentAppInfo, setCurrentAppInfo] = useState<CurrentAppInfo | null>(
    null
  );
  ...
  const handleRecordedAudio = useCallback(
    async (payload: StopRecordingResponse) => {
      ...
      try {
        const info = await invoke<CurrentAppInfo>("get_current_app_info");
        setCurrentAppInfo(info);
      } catch (error) {
        console.error("Failed to retrieve current application info", error);
      }
    },
    []
  );
  ...
  return (
    <Dialog open onClose={closeAppInfoDialog} maxWidth="xs" fullWidth>
      <DialogTitle>Current application</DialogTitle>
      <DialogContent>
        <Stack spacing={2} alignItems="center">
          <Avatar
            src={`data:image/jpeg;base64,${currentAppInfo.iconBase64}`}
            alt={currentAppInfo.appName}
      ...
      <DialogActions>
        <Button onClick={closeAppInfoDialog}>Close</Button>
      </DialogActions>
    </Dialog>
  );
};
