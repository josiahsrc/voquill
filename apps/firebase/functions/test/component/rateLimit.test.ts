...
describe("rate limiting", () => {
	it("increments my limits when I call", async () => {
		...
		const prevLimit =
			(await admin.database().ref(`/limits/${creds.id}`).get()).val() || 0;
	...
	it("disables my account when I exceed rate limit", async () => {
		const runTest = async (fn: () => Promise<unknown>) => {
			...
			await expect(fn()).rejects.toThrow(/rate limit exceeded/i);
		...
		await runTest(() =>
			invokeHandler("user/setMyUser", { value: buildUser() }),
		);
		await runTest(() => invokeHandler("member/getMyMember", {}));
		await runTest(() => invokeHandler("term/listMyTerms", {}));
		await runTest(() =>
			invokeHandler("term/upsertMyTerm", { term: buildTerm() }),
		);
		await runTest(() =>
			invokeHandler("term/deleteMyTerm", { termId: "someTermId" }),
		);
	});
});
