...
export const validateDemoUsageWithinLimits = async (args: {
  ...
  const [ipUsage, browserUsage] = await Promise.all([
    firemix().get(mixpath.usageByIp(args.ip)),
    firemix().get(mixpath.usageByBrowserId(args.browserId)),
  ]);
...
export const incrementDemoUsage = async (args: {
  ...
  await Promise.all([
    firemix().set(mixpath.usageByIp(args.ip), updatedIpUsage),
    firemix().set(mixpath.usageByBrowserId(args.browserId), updatedBrowserUsage),
  ]);
};
